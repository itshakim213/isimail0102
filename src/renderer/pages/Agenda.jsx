...
function Agenda() {
  ...
  const { data:eventsData, isLoading, isError } = useQuery('events', async () => {
    const response = await axios.get('http://localhost:4001/api/agenda/events',{
      headers: {
  ...
  return (
    ...
    >
      ...
      {
        (selectedEvent === null) ? (
          <EventForm date={date} eventToUpdate={eventToUpdate} evenements={evenements} setEvenements={setEvenements} />
        ) : (
